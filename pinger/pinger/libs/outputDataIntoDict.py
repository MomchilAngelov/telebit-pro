...
class Outputter():
	...
	def appendItemToOutputter(self, output):
		...
		items[output['ip'].strip()+'.packet_loss'] = {
			'value': packet_loss,
			'units': '%',
			'name': '[{0} -> {1}] ICMP ping: packet loss'.format(output['ip'], output['address']),
			'type': 'float',
			'timestamp': int(timestamp), 
			'domain': output['address'],
			'requesting_application': output['application_name_human_name']
	...
	def appendItemToOutputterHTML(self, output):
		...
		items['{0}.breakThrough'.format(output['idx'])] = {
			'timestamp': output['timestamp'],
			'Value': 1 if output['breakThrough'] else 0,
			'address': output['name'],
			'type': 'boolean',
			'units': None
	...
	def getResult(self, outputFormat = None):
